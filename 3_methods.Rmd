...
# Method1: Analysis of evaluation metrics by considering optics zero points as a single cluster
...
# Method2: Analysis of evaluation metrics after removal of optics zero points

```{r clustering_module_1, class.source='fold-show', summary=T}
...
```{r evaluation_module_1, class.source='fold-show', summary=T}
#' Evaluation module
#'
#' Takes as input the cluster memberships produced by a single run of `cluster_module`, and the
#' parameters used by `cluster_module`. Returns a list of evaluation metrics for that run of
...
```{r run_all_function_1, class.source='fold-show', summary=T}
run_all = function(data, 
                   labels,
                   k=seq(2, 10), 
                   agg = c('average','single','complete','ward','mcquitty'),
    ...
    bind_rows(lapply(algos, function(algo) {
      if (algo == "OPTICS") {
          ...
          cvec = cluster_module(data.dist, algo="OPTICS", xi=xival, minPts=mpval)
           ...
           cvec = cvec[cvec>0]
           print(paste(xival, mpval))
           if(length(unique(cvec))>1){
             c(list(param.algo="OPTICS", param.xi=xival, param.minPts=mpval), evaluation_module(d.dist, cvec), cvec_val=as.list(cvec))
           }
...
```{r iris_dataset_1, warning=FALSE, cache=TRUE}
data(iris)
dim(iris)
levels(iris$Species)
head(iris)
...
```{r run_all_iris_1, warning=FALSE, cache=TRUE}
run_output_iris <- run_all(data=iris[1:4], labels=iris$Species)
eval_res_iris <-  dplyr::select(run_output_iris, !starts_with("sample"))
reactable(eval_res_iris,
          columns = list(cohesiveness = colDef(format = colFormat(locales = "en-US", digits = 2)),
...
```{r heatmap_iris_1, warning=FALSE, cache=TRUE}
# corr_mat <- round(cor(eval_res[,7:ncol(eval_res)], method = "spearman", use = "pairwise.complete.obs"),2)
corr_mat_iris <- round(cor(dplyr::select(eval_res_iris, !starts_with("param")), 
                            method = "spearman", use = "pairwise.complete.obs"),2)
...
```{r nmi_corr_iris_1, warning=FALSE, cache=TRUE}
melted_corr_mat_iris %>% 
  filter(Var1 == "NMI" & !Var2 %in% c("NMI","ARI")) %>% 
  slice_max(value, n=5)
```
...
```{r iris_k_3_1, warning=FALSE, cache=TRUE}
eval_res_iris_k_3 <- eval_res_iris %>% 
  dplyr::filter(param.k == 3) %>%
  dplyr::select(!starts_with("param"))
...
# Method3: Analysis of evaluation metrics after randomly assigning optics zero points to nearest clusters

```{r clustering_module_2, class.source='fold-show', summary=T}
#' Clustering module
#'
#' Takes as input a distance matrix and runs clustering algorithms with a given set of parameters.
#'
...
```{r evaluation_module_2, class.source='fold-show', summary=T}
#' Evaluation module
#'
#' Takes as input the cluster memberships produced by a single run of `cluster_module`, and the
#' parameters used by `cluster_module`. Returns a list of evaluation metrics for that run of
...
```{r run_all_function_2, class.source='fold-show', summary=T}
run_all = function(data, 
                   labels,
                   k=seq(2, 10), 
                   agg = c('average','single','complete','ward','mcquitty'),
  ...
  df2 = bind_rows(apply(optics_params, 1, function(row) {
      bind_rows(lapply(algos, function(algo) {
        if (algo == "OPTICS") {
          ...
          if(length(unique(cvec[cvec > 0])) > 1) {
            # Get labeled data (non-noise points)
            labeled_indices <- which(cvec > 0)
            labeled_data <- data[labeled_indices, ]
            labeled_labels <- cvec[labeled_indices]
            
            # Assign noise points using k-NN
            zero_indices <- which(cvec == 0)
            if(length(zero_indices) > 0) {
              noise_points <- data[zero_indices, ]
              
              # Find the k-nearest neighbors (k = 3)
              k <- 3
              knn_result <- get.knnx(labeled_data, noise_points, k = k)
              
              # Assign each noise point to the most frequent cluster label among its k neighbors
              for (i in seq_along(zero_indices)) {
                neighbor_labels <- labeled_labels[knn_result$nn.index[i, ]]
                cvec[zero_indices[i]] <- as.numeric(names(which.max(table(neighbor_labels))))
              }
            }
            
            c(list(param.algo="OPTICS", param.xi=xival, param.minPts=mpval), 
              evaluation_module(data.dist, cvec), 
              cvec_val=as.list(cvec))
          }
        }
      }))
    }))
...
```{r iris_dataset_2, warning=FALSE, cache=TRUE}
data(iris)
dim(iris)
levels(iris$Species)
head(iris)
...
```{r run_all_iris_3, warning=FALSE, cache=TRUE}
run_output_iris <- run_all(data=iris[1:4], labels=iris$Species)
eval_res_iris <-  dplyr::select(run_output_iris, !starts_with("sample"))
reactable(eval_res_iris,
          columns = list(cohesiveness = colDef(format = colFormat(locales = "en-US", digits = 2)),
...
```
## Create a rank correlation to compare all the evaluation metrics

```{r heatmap_iris_3, warning=FALSE, cache=TRUE}
# corr_mat <- round(cor(eval_res[,7:ncol(eval_res)], method = "spearman", use = "pairwise.complete.obs"),2)
corr_mat_iris <- round(cor(dplyr::select(eval_res_iris, !starts_with("param")), 
                            method = "spearman", use = "pairwise.complete.obs"),2)
...
```{r nmi_corr_iris_3, warning=FALSE, cache=TRUE}
melted_corr_mat_iris %>% 
  filter(Var1 == "NMI" & !Var2 %in% c("NMI","ARI")) %>% 
  slice_max(value, n=5)
```
...
```{r iris_k_4, warning=FALSE, cache=TRUE}
eval_res_iris_k_3 <- eval_res_iris %>% 
  dplyr::filter(param.k == 3) %>%
  dplyr::select(!starts_with("param"))
