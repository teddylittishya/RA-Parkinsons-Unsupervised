...
run_output <- run_all(data=wine, labels=wine$class)
...
# Compare cluster assignments to the true classes of Wine dataset 

## Display the NMI and ARI indices of the first CVEC with true Wine labels

```{r true_indices, warning=FALSE, cache=TRUE}
aricode::NMI(cvec_mat[1,], wine$class)
mclust::adjustedRandIndex(cvec_mat[1,], wine$class)
```

## Display the NMI values for runs with k=3

```{r k_3, warning=FALSE, cache=TRUE}
eval_res_k_3 <- eval_res %>% 
  dplyr::filter(param.k == 3) %>%
  dplyr::select(starts_with('param') | c(NMI, ARI))

reactable(eval_res_k_3,
