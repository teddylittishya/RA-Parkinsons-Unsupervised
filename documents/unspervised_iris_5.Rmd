...
run_all = function(data, 
  ...
  data.dist = as.dist(1 - data.rf$proximity)
    ...
    df1 = bind_rows(unlist(df1, recursive=F))
     ...
     df2 = apply(optics_params, 1, function(row) {
       ...
       nmi = aricode::NMI(as.factor(cvec), labels)
       ari = mclust::adjustedRandIndex(as.factor(cvec), labels)
       c(param.algo="OPTICS", param.xi=xival, param.minPts=mpval,
         evaluation_module(data.dist, cvec, parameter = row),
         NMI=nmi,
         ARI=ari,
         cvec)
     })
